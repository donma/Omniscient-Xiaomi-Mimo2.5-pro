...
【樣板定位】
- 樣板 ID：T035
- 網站主題：Cloud Consulting / 雲端顧問
- 產業分類：科技商務
- 核心描述：專業雲端顧問服務，AWS、Azure、GCP 雲端架構規劃與遷移，幫助企業實現雲端轉型。
- 技術框架：Bootstrap 5
- 首頁 title 參考：雲端顧問 | 企業雲端轉型專家
- Hero 主標方向：讓您的企業 / 翱翔雲端
- Meta description 參考：專業雲端顧問服務，AWS、Azure、GCP 雲端架構規劃與遷移，幫助企業實現雲端轉型。
...
【每個頁面的內容職責】
- index.html：導覽文案為「CloudPro」。首頁：建立第一印象，包含 hero、核心賣點、代表性內容區塊、轉換 CTA 與完整 footer。 如果檔名與實際頁面定位略有差異，請以導覽文案與整體樣板脈絡優先。
...
- booking.html：導覽文案為「免費評估」。預約/諮詢頁：提供表單、預約流程、方案說明與行動呼籲。 如果檔名與實際頁面定位略有差異，請以導覽文案與整體樣板脈絡優先。
- process.html：流程頁：清楚說明合作步驟、服務節點、時間安排與交付方式。
- blog.html：導覽文案為「雲端知識」。文章列表頁：整理內容分類、精選文章卡片與延伸閱讀 CTA。 如果檔名與實際頁面定位略有差異，請以導覽文案與整體樣板脈絡優先。
